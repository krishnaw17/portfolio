...
export function ExperienceAdmin() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get<{ experience: Experience[] }>('/admin/content', true);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isNew) {
        await api.post('/admin/content/experience', editing, true);
      } else {
        await api.put(`/admin/content/experience/${editing.id}`, editing, true);
      }
      setEditing(null);
      setIsNew(false);
      await fetchItems();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/admin/content/experience/${id}`, true);
      await fetchItems();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => { setEditing({ ...EMPTY, id: `exp-${Date.now()}` }); setIsNew(true); }}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm pt-[5vh] overflow-y-auto pb-10">
          <div className="w-full max-w-2xl rounded-lg border border-neutral-800 bg-[#0f0f14] p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-semibold">{isNew ? 'New Experience' : 'Edit Experience'}</h2>
            ...
            <div className="space-y-3">
              ...
              <Field label="Achievements (one per line)" value={editing.achievements.join('\n')} onChange={(v) => setEditing({ ...editing, achievements: v.split('\n').filter(Boolean) })} textarea />
              <Field label="Technologies (comma-separated)" value={editing.technologies.join(', ')} onChange={(v) => setEditing({ ...editing, technologies: v.split(',').map((t) => t.trim()).filter(Boolean) })} />
            </div>
      ...
      <div className="space-y-2">
        {items.length === 0 && <p className="text-neutral-600 text-sm">No experience entries.</p>}
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between rounded-lg border border-neutral-800/50 bg-neutral-900/30 px-4 py-3">
            <div className="min-w-0">
              <div className="text-sm font-medium truncate">{item.role}</div>
              <div className="text-xs text-neutral-500 mt-0.5">{item.company} · {item.period}</div>
            </div>
